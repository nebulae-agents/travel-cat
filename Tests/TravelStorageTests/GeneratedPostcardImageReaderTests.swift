...
final class GeneratedPostcardImageReaderTests: XCTestCase {
  func testReadsOnlyBoundedRegularOwnedBytes() throws {
    ...
    let reader = GeneratedPostcardImageReader(allowedRoot: root, maximumBytes: 3)
  ...
  func testRejectsLinksTraversalAndReplacement() throws {
    ...
    let file = root.appendingPathComponent("ink.png")
    try Data([1, 2, 3]).write(to: file)
    let reader = GeneratedPostcardImageReader(allowedRoot: root)
    ...
    let replacing = GeneratedPostcardImageReader(allowedRoot: root, beforeRevalidation: {
